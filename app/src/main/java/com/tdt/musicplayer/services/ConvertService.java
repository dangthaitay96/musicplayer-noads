...
public class ConvertService {
  ...
  public void download(String youtubeUrl) {
    ...
    Request request = new Request.Builder()
            .url("http://10.0.2.2:8080/convert")
            .post(formBody)
            .build();
    ...
    client.newCall(request).enqueue(new Callback() {
      ...
      public void onFailure(Call call, IOException e) {
        ...
        if (onFinish != null) onFinish.run(); // ✅ gọi khi lỗi
      }
      ...
      public void onResponse(Call call, Response response) throws IOException {
        ...
        try (InputStream inputStream = response.body().byteStream();
          ...
          // ✅ Quét lại để MediaStore nhận diện
          MediaScannerConnection.scanFile(
                  context,
                  new String[]{ outFile.getAbsolutePath() },
                  new String[]{ "audio/mpeg" },
                  (path, uri) -> Log.d("ConvertService", "File scanned: " + path + ", URI: " + uri)
          );
        ...
        if (onFinish != null) onFinish.run(); // ✅ gọi sau khi hoàn thành
      }
    });
  }
